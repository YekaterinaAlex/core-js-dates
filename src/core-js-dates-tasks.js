...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const difMsec = end.getTime() - start.getTime();
  return difMsec / (1000 * 60 * 60 * 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    dateObj.getTime() >= start.getTime() && dateObj.getTime() <= end.getTime()
  );
}
...
function formatDate(date) {
  ...
  hours %= 12;
  if (hours === 0) hours = 12;
  const minNew = String(min).padStart(2, '0');
  const secNew = String(sec).padStart(2, '0');
  return `${month}/${day}/${year}, ${hours}:${minNew}:${secNew} ${ampm}`;
}
